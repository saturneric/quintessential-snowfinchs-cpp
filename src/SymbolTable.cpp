...
namespace {
auto GetSemanticKeyByScope(int scope, const std::string &name) {
  return "sc_" + std::to_string(scope) + "_" + name;
};
};  // namespace
...
auto SymbolTable::AddSymbol(SymbolType type, const std::string &name,
  ...
  return AddSymbol(type, index_++, name, value, unique);
}

auto SymbolTable::AddASTSymbol(const std::string &name,
                               const std::string &value) -> SymbolPtr {
  return AddSymbol(SymbolType::kAST, name, value, false);
}

auto SymbolTable::AddSemanticSymbol(int scope, const std::string &name,
                                    const std::string &value) -> SymbolPtr {
  const auto key = GetSemanticKeyByScope(scope, name);
  return AddSymbol(SymbolType::kSEMANTIC, key, value);
}
...
auto SymbolTable::SearchSymbol(SymbolType type, const std::string &name) const
    -> SymbolPtr {
  if (table_.count(type) == 0) return nullptr;
...
auto SymbolTable::SearchSemanticSymbol(int scope, const std::string &name) const
    -> SymbolPtr {
  const auto key = GetSemanticKeyByScope(scope, name);
  return SearchSymbol(SymbolType::kSEMANTIC, key);
}
